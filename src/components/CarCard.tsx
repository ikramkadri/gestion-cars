...
import { 
  // CarType is imported from '../features/cars/types/car.types';
  Heart, 
  ChevronRight, 
  ChevronLeft, 
  Gauge, 
...
const CarCard = ({ car }: CarCardProps) => {
  const images = car.imagesUrls && car.imagesUrls.length > 0 
    ? car.imagesUrls.filter((url): url is string => url !== null)
    : [car.mainImageUrl || "https://images.unsplash.com/photo-1533473359331-0135ef1b58bf?q=80&w=150"];
  ...
  const handleWhatsApp = () => {
    ...
    const message = encodeURIComponent(`مرحباً، أود الاستفسار عن سيارة ${car.make} ${car.model}.`);
      ...
      <div className="group relative flex flex-col md:flex-row w-full max-w-4xl bg-white rounded-[2.5rem] overflow-hidden shadow-2xl hover:shadow-[0_40px_100px_-20px_rgba(0,0,0,0.2)] transition-all duration-700 border border-slate-200 h-auto md:min-h-[22rem]">
        ...
        <div className="flex-1 p-8 flex flex-col justify-between bg-white relative">
          ...
          <div className="flex justify-between items-start">
            <div className="space-y-1.5">
              <div className="flex items-center gap-2.5">
                <span className="text-[10px] font-black text-blue-700 bg-blue-100 px-2.5 py-1 rounded-lg uppercase tracking-widest shadow-sm">{car.condition === 'New' ? 'جديد' : 'مستعمل'} {car.year}</span>
                <span className="flex items-center gap-1 text-slate-500 text-[11px] font-bold">
                  <Eye size={13} className="text-slate-400" /> {car.viewCount} مشاهدة
                </span>
              </div>
              ...
              <div className="flex items-center gap-4 text-slate-500 text-xs font-semibold">
                <span className="flex items-center gap-1.5"><MapPin size={15} className="text-blue-500" /> {car.location}</span>
                <span className="flex items-center gap-1.5"><Calendar size={15} className="text-blue-500" /> {car.year}</span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-3 gap-6 py-6">
            {[
              { label: 'المسافة', value: `${car.mileage} كم`, icon: <Gauge size={18} />, delay: 'delay-0' },
              { label: 'المحرك', value: car.engineSize || "N/A", icon: <Settings size={18} />, delay: 'delay-100' },
              { label: 'الوقود', value: car.fuel, icon: <Fuel size={18} />, delay: 'delay-200' }
            ].map((item, idx) => (
            ...
            <div className="grid grid-cols-2 gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
               ...
               <div className="flex items-center gap-3">
                 ...
                 <div>
                   ...
                   <p className="text-xs font-bold text-slate-700">{car.color || 'غير محدد'}</p>
                 </div>
               </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-6 pt-5 border-t-2 border-slate-50">
            <div className="flex flex-col transition-all duration-500 group-hover:scale-110 origin-right">
               ...
               <div className="flex items-baseline gap-1.5">
                 <span className="text-4xl font-black text-slate-900 tracking-tight">{(car.price / 1000000).toFixed(1)}</span>
