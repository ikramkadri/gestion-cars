          ...
          <table className="w-full text-right">
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredCars.map((car) => (
                <tr key={car._id} className="hover:bg-slate-50/50 transition-all group">
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-8 rounded-lg bg-slate-100 overflow-hidden">
                      ...
                      <span className="font-black text-slate-800">{car.make} {car.model}</span>
                    </div>
                  </td>
                  <td className="px-8 py-5 font-black text-slate-900 tabular-nums text-right">
                    {car.price.toLocaleString()} <span className="text-[10px] text-slate-400">دج</span>
                  </td>
                  <td className="px-8 py-5 text-right">
                    <span className={`px-3 py-1 rounded-lg text-[10px] font-black ${car.status === "Sold" ? 'bg-rose-50 text-rose-600 border border-rose-100' : 'bg-slate-100 text-slate-500 border border-slate-200'}`}>
